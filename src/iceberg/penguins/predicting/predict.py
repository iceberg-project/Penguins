...
class Pipe:
    ...
    def import_model(self):
        ...
        if 'unetr' in self.opt.name:
            self.opt.model='unetr'
        elif 'unet' in self.opt.name:
            self.opt.model = 'unet'
        self.opt.which_epoch=self.epoch
        self.opt.serial_batches = True  # no shuffle
        self.opt.no_flip = True  # no flip
        self.opt.no_dropout = True
        self.opt.gpu_ids = [0]
        self.network = create_model(self.opt)
        self.network.eval()
        #self.opt = opt 
    ...
    def png_predict(self,im):
        last = time.time()
        #self.opt.step = self.opt.step#128
        #self.opt.size = self.opt.size#256
        w,h,c = im.shape
        patches = png2patches(im,self.opt.step,self.opt.size)
        print(patches.shape)
        ...
        outpng = patches2png_legacy(out,w,h,self.opt.step,self.opt.size)
        print('merging')
...
#        self.eval_dir_J('/nfs/bigbox/hieule/GAN/data/Penguins/Test/B')
    def test_single_png(self,impath):
        ...
        print "pass image reading correctly"
        sys.stdout.flush()
        outpng = self.png_predict(inpng)
        print "pass png_predict"
        sys.stdout.flush()
        outim = show_plainmask_on_image(inpng,outpng)
