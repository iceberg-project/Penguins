...
#        print('-----------------------------------------------')
    ...
    def backward_G(self):
        ...
        #regression loss:
        self.loss_G_regression = self.criterion(self.output[self.isweak==1,:,:,:].mean(dim=0),self.GT[self.isweak==1,:,:,:].mean(dim=0)) * self.opt.lambda_regression
        coutput = torch.clamp(self.output[self.isweak==1,:,:,:]/2+0.5,0.00001,1)
        self.loss_G_entropy = -(coutput*torch.log(coutput)).mean() * self.opt.lambda_entropy
        #blurry_prediction =  self.output[(self.output>0) & (self.output<1)]
        #self.loss_G_entropy = -blurry_prediction
        self.loss_G = self.loss_G_segmentation + self.loss_G_regression + self.loss_G_entropy
    ...
    def get_current_errors(self):
        ...
        return OrderedDict([('G_Loss', self.loss_G.detach().cpu()),
                            ...
                            ('G_Entropy',self.loss_G_entropy.detach().cpu()),
