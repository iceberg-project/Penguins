...
class SUnetPriorModel(BaseModel):
    def name(self):
        return 'SensitiveUnetModelWithPriorBranch'
    def eval(self):
        self.netG.eval()
...
#        print('-----------------------------------------------')
    ...
    def set_input(self, input):
        ...
        mean_mask = 1-(self.prior>0).type(torch.FloatTensor).view(self.batch_size,-1).mean(dim=1).cuda(self.gpu_ids[0], async=True)

        self.mean_mask = mean_mask.view(self.input.shape[0],1,1,1).repeat(1,1,self.input.shape[2],self.input.shape[3])
    ...
    def forward(self):
        ...
        self.output = self.netG(torch.cat((self.input,self.mean_mask),1),self.prior)
    ...
    def get_current_visuals(self):
       	self.visual_names = ['input','output','GT','prior','mean_mask'] 
