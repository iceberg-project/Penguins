...
print "test:"
print test_list
train_list = [imlist[i] for i in trainidx]
print "train:"
print train_list

file = open(opt.im_fold+"/test1","w")
for i in test_list:
    file.write(i[:-4]+"$")
file.close()
...
file = open(opt.im_fold+"/train1_all","w")
for i in train_list:
    file.write(i[:-4]+"$")
file.close()
...
for ratio in ratioa:
    ...
    for iter in range(0,5):
        ...
        file = open(opt.im_fold+"/train1"+str(ratio)+'_'+str(iter),"w")
        for i in tlist:
            file.write(i[:-4]+"$")
        file.close()
