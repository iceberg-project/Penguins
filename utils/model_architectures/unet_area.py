...
class up(nn.Module):
    def __init__(self, in_ch, out_ch, drop_rate=0.3, bilinear=False):
...
class UNet_Area(nn.Module):
    def __init__(self, scale=32, n_channels=1, n_classes=1, drop_rate=0.3):
        super(UNet_Area, self).__init__()
                
        # Unet part for heatmap
        self.inc = inconv(n_channels, scale)
        self.down1 = down(scale, scale*2, drop_rate)
        self.down2 = down(scale*2, scale*4, drop_rate)
        self.down3 = down(scale*4, scale*8, drop_rate)
        self.down4 = down(scale*8, scale*8, drop_rate)
        ...
        self.down5 = down(1, scale, drop_rate)
        self.down6 = down(scale, scale*2, drop_rate)
        self.down7 = down(scale*2, scale*4, drop_rate)
